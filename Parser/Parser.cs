...
namespace Compiler.Parser
{
    public class Parser
    {
        ...
        public void Parse()
        {
            ...
            Stack.InsertRange(0, p.Where(x => !(x is SemanticAction)).Select(x => new ParsingNode { Parent = parent, Expression = Expression.Create(x), SubProduction = p, Parser = this }));
            ...
            while (Stack.First().Expression is NonTerminalExpression || (Stack.First().Expression is TerminalExpression te1 && te1.TokenType != TokenType.EndMarker))
            {
                ...
                else if (Stack.First().Expression is NonTerminalExpression nte)
                {
                    ...
                    Stack.InsertRange(0, subProduction.Where(x => !(x is SemanticAction)).Select(x => new ParsingNode { Parent = nte.Tree, Expression = Expression.Create(x), Parser = this }));
                }
            }
        ...
        private SubProduction GetAProduction()
        {
            foreach (Production p in Parser.Grammar)
            {
                ...
                {
                    if (subProduction.First(x => !(x is SemanticAction)).First().Any(x => x.TokenType == Current.Type))
                    {
